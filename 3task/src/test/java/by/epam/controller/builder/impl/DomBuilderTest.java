...
public class DomBuilderTest {
    ...
    public void build() throws SAXException, IOException, XMLStreamException {
        List<Candy> candies = Director.build("candy.xml", domBuilder);
        assertEquals(3, candies.size());
    }

    @Test(expected = IOException.class)
    public void getException() throws SAXException, IOException, XMLStreamException {
        Director.build("cany.xml", domBuilder);
    }
}